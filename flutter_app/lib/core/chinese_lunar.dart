...
class ChineseLunar {
  // Data source: common lunarInfo encoding (1900-2100).
  ...
  static LunarDate fromDate(DateTime date) {
    final normalized = DateTime(date.year, date.month, date.day);
    var offsetDays = normalized.difference(_baseDate).inDays;
    if (offsetDays < 0) {
      // Fallback: clamp to base date range.
      offsetDays = 0;
    }

    var lunarYear = 1900;
    var daysInYear = _yearDays(lunarYear);
    while (lunarYear < 2101 && offsetDays >= daysInYear) {
      offsetDays -= daysInYear;
      lunarYear += 1;
      daysInYear = _yearDays(lunarYear);
    }

    final leapMonth = _leapMonth(lunarYear);
    var isLeap = false;
    var lunarMonth = 1;
    var daysInMonth = _monthDays(lunarYear, lunarMonth);

    while (lunarMonth <= 12 && offsetDays >= daysInMonth) {
  ...
  static String format(DateTime date) {
    ...
    final monthLabel = _monthLabel(lunar.month, lunar.isLeapMonth);
    final dayLabel = _dayLabel(lunar.day);
    return '$monthLabel$dayLabel';
  }

  static int _yearDays(int year) {
    var sum = 12 * 29;
    final info = _info(year);
  ...
  static int _info(int year) {
    final index = year - 1900;
    if (index < 0 || index >= _lunarInfo.length) return _lunarInfo.first;
    return _lunarInfo[index];
  }

  static String _monthLabel(int month, bool isLeap) {
    const labels = <String>[
      '',
      '正月',
      '二月',
    ...
    final base = month >= 1 && month <= 12 ? labels[month] : '$month月';
  ...
  static String _dayLabel(int day) {
    ...
    if (day >= 1 && day < labels.length) return labels[day];
    return '$day';
  }
}
