...
class ApiClient {
  ...
  Future<HolidayCnYear> getHolidayCnYear(int year) async {
    final res = await http.get(_uri('/api/holidays/$year'), headers: _headers());
    _throwIfError(res);
    final json = _decode(res);
  ...
  Future<void> deleteTask(String id) async {
    final res = await http.delete(_uri('/api/v2/tasks/$id'), headers: _headers());
    _throwIfError(res);
  }

  Future<int> emptyTrash() async {
    final res = await http.delete(_uri('/api/v2/tasks/trash/empty'), headers: _headers());
    _throwIfError(res);
    final json = _decode(res);
  ...
  Future<TaskAttachment> uploadAttachment({
    ...
    final request = http.MultipartRequest('POST', _uri('/api/v2/tasks/$taskId/attachments'));
  ...
  Future<List<ChecklistList>> getChecklists() async {
    ...
    if (list is List) {
      return list.whereType<Map<String, dynamic>>().map(ChecklistList.fromJson).toList();
    }
  ...
  Future<void> deleteChecklist(int id) async {
    final res = await http.delete(_uri('/api/checklists/$id'), headers: _headers());
    _throwIfError(res);
  }

  Future<List<ChecklistItem>> getChecklistItems(int listId) async {
    final res = await http.get(_uri('/api/checklists/$listId/items'), headers: _headers());
    _throwIfError(res);
    final json = _decode(res);
    final list = json['items'];
    if (list is List) {
      return list.whereType<Map<String, dynamic>>().map(ChecklistItem.fromJson).toList();
    }
  ...
  Future<void> deleteChecklistItem({
    ...
    final res = await http.delete(_uri('/api/checklists/$listId/items/$itemId'), headers: _headers());
    _throwIfError(res);
  }
  ...
  Future<List<TimeActivity>> getActivities({
    ...
    final uri = _uri('/api/v2/time/activities').replace(queryParameters: params);
    final res = await http.get(uri, headers: _headers());
    _throwIfError(res);
    final json = _decode(res);
    final list = json['activities'];
    if (list is List) {
      return list.whereType<Map<String, dynamic>>().map(TimeActivity.fromJson).toList();
    }
    return <TimeActivity>[];
  }
  ...
  Future<void> deleteActivity(String id) async {
    final res = await http.delete(_uri('/api/v2/time/activities/$id'), headers: _headers());
    _throwIfError(res);
  }
  ...
  Future<List<TimeEntry>> getEntries({
    ...
    if (activityId != null && activityId.isNotEmpty) params['activity_id'] = activityId;
    ...
    if (list is List) {
      return list.whereType<Map<String, dynamic>>().map(TimeEntry.fromJson).toList();
    }
    return <TimeEntry>[];
  }

  Future<List<TimeEntry>> getRunningEntries() async {
    final res = await http.get(_uri('/api/v2/time/entries/running'), headers: _headers());
    _throwIfError(res);
    final json = _decode(res);
    final list = json['entries'];
    if (list is List) {
  ...
  Future<void> deleteEntry(String id) async {
    final res = await http.delete(_uri('/api/v2/time/entries/$id'), headers: _headers());
    _throwIfError(res);
  }
  ...
  Future<PomodoroSettings> getPomodoroSettings() async {
    final res = await http.get(_uri('/api/pomodoro/settings'), headers: _headers());
    _throwIfError(res);
    final json = _decode(res);
  ...
  Future<PomodoroSettings> savePomodoroSettings(PomodoroSettings settings) async {
    final res = await http.post(
  ...
  Future<PomodoroState?> getPomodoroState() async {
    final res = await http.get(_uri('/api/pomodoro/state'), headers: _headers());
    _throwIfError(res);
    final json = _decode(res);
  ...
  Future<List<PomodoroSession>> getPomodoroSessions({int limit = 50}) async {
    ...
    if (list is List) {
      return list.whereType<Map<String, dynamic>>().map(PomodoroSession.fromJson).toList();
    }
  ...
  Future<String> refreshInviteCode() async {
    final res = await http.post(_uri('/api/admin/invite/refresh'), headers: _headers(), body: jsonEncode({}));
    _throwIfError(res);
    final json = _decode(res);
    return (json['code'] ?? '').toString();
  }
