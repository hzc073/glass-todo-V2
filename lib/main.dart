...
Future<void> main() async {
  ...
  runApp(GlassTodoApp(config: config, authStore: authStore, settings: settings));
}
...
class _GlassTodoAppState extends State<GlassTodoApp> {
  ...
  void initState() {
    super.initState();
    _apiBaseUrl = widget.settings.apiBaseUrlOverride ?? widget.config.apiBaseUrl;
  ...
  Future<void> _exportData(BuildContext context) async {
    try {
      ...
      final downloaded = await downloadTextFile(filename: filename, content: pretty, mimeType: 'application/json');
      await Clipboard.setData(ClipboardData(text: pretty));
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(downloaded ? '已导出 JSON，并复制到剪贴板。' : '已复制 JSON 到剪贴板。')),
      );
    } on UnauthorizedException {
      await _handleLogout();
    } catch (_) {
  ...
  Future<void> _importData(BuildContext context) async {
    try {
      ...
      if (decoded is! Map) {
        if (!context.mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('JSON 格式不正确。')));
        return;
      }
  ...
  Future<void> _clearCompletedTasks(BuildContext context, int retentionDays) async {
    try {
      final purged = await _apiClient.cleanupCompletedTasks(retentionDays: retentionDays);
      if (!mounted) return;
      setState(() => _taskPageKey = UniqueKey());
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('已清空 $purged 个已完成任务。')));
    } on UnauthorizedException {
      await _handleLogout();
    } catch (_) {
      if (!context.mounted) return;
  ...
  Future<void> _openBackendSettings() async {
    final navContext = _navKey.currentContext;
    if (navContext == null) return;
    final current = _apiBaseUrl.trim().isEmpty ? widget.config.apiBaseUrl : _apiBaseUrl;
    final next = await showBackendSettingsDialog(navContext, initialValue: current);
    if (next == null) return;
